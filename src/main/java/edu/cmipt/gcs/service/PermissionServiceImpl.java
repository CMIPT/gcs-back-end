...
/**
 * 权限检查服务实现类 该类负责检查用户对仓库、活动和评论的操作权限。
 *
 * <p>读取权限： 如果仓库是public的，则任何用户都可以读取仓库、活动和评论。 如果仓库是private的，则只有仓库创建者和协作者可以读取仓库、活动和评论。
 *
 * <p>修改权限： 对于仓库，只有仓库创建者和协作者可以修改仓库组件，目前只有标签组件。 对于活动，仓库创建者和协作者可以修改活动，如果活动所属仓库是public的，则活动创建者也可以修改活动。
...
public class PermissionServiceImpl implements PermissionService {
  ...
  public void checkActivityOperationValidity(
    ...
    if (operationTypeEnum == OperationTypeEnum.READ
        || operationTypeEnum == OperationTypeEnum.ATTACH_ACTIVITY) {
      ...
      try {
        ...
        if (operationTypeEnum == OperationTypeEnum.MODIFY_ACTIVITY
            && idInToken.equals(activityPO.getCreatorId())) {
        ...
        if (activityPO.getGmtLocked() != null) {
          // Activity is locked, throw ACTIVITY_LOCKED exception, so that only repository creator
          // and collaborators can modify activity
  ...
  public void checkCommentOperationValidity(
    ...
    try {
      ...
      if (ErrorCodeEnum.ACCESS_DENIED.equals(e.getCode())
          && idInToken.equals(commentPO.getCreatorId())) {
