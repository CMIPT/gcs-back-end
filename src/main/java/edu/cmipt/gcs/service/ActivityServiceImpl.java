...
public class ActivityServiceImpl extends ServiceImpl<ActivityMapper, ActivityPO> implements ActivityService {
    ...
    @Autowired
    private ActivityAssignLabelService activityAssignLabelService;
    @Autowired
    private ActivityDesignateAssigneeService activityDesignateAssigneeService;
    @Autowired
    ...
    @Override
    public Page<ActivityDetailDTO> pageActivities(ActivityQueryDTO activityQueryDTO, Page<ActivityDetailDTO> page) {
        // 连表分页查询
        var queryWrapper =
        ...
        if (activityQueryDTO.labels() != null && !activityQueryDTO.labels().isEmpty()) {
            queryWrapper.in(ActivityPO::getId, ActivityAssignLabelPO.class, sub -> sub
        ...
        if (activityQueryDTO.assignees() != null && !activityQueryDTO.assignees().isEmpty()) {
            queryWrapper.in(ActivityPO::getId, ActivityDesignateAssigneePO.class, sub -> sub
                    .select(ActivityDesignateAssigneePO::getActivityId)
            ...
            switch (activityQueryDTO.orderBy()) {
                case GMT_CREATED:
                    queryWrapper.orderBy(true, activityQueryDTO.isAsc(),
                            ActivityPO::getGmtCreated);
                    break;
                case GMT_UPDATED:
                    queryWrapper.orderBy(true, activityQueryDTO.isAsc(),
                            ActivityPO::getGmtUpdated);
                    break;
//                case TOTAL_COMMENTS:
//                    queryWrapper.orderBy(true, activityQueryDTO.isAsc(),
...
//                    break; // todo 暂不支持按评论数排序
        ...
        Page<ActivityDetailDTO>  activityDetailDTOPage = activityMapper.selectJoinPage(page, ActivityDetailDTO.class, queryWrapper);
        ...
        if(activityDetailDTOPage.getRecords().isEmpty())
            return activityDetailDTOPage;
        // 提取ActivityPO的ID列表
        List<Long> activityIds = activityDetailDTOPage.getRecords().stream()
                .map(ActivityDetailDTO::getId)
                .toList();
        logger.info("activityIds: {}", activityIds);
        //  批量查询每个Activity的标签、指定的参与者和评论数
        Map<Long, List<LabelVO>> labelMap = activityAssignLabelService.getLabelsByActivityIds(activityIds);
        Map<Long, List<AssigneeVO>> assigneeMap = activityDesignateAssigneeService.getAssigneesByActivityIds(activityIds);
        ...
        page.getRecords().forEach(activity -> {
            ...
            activity.setAssignees(assigneeMap.getOrDefault(activity.getId(), Collections.emptyList()));
    ...
    public ActivityDetailDTO getDetailedOneById(Long id) {
        ...
        ActivityDetailDTO activityDetailDTO = activityMapper.selectJoinOne(ActivityDetailDTO.class, queryWrapper);
        if(activityDetailDTO != null) {
            Long commentCount = commentMapper.selectCount(
                    Wrappers.lambdaQuery(CommentPO.class)
                            .eq(CommentPO::getActivityId, id)
            );
            List<LabelVO> labelDTOList = activityAssignLabelService.getLabelsByActivityIds(List.of(id))
                    .getOrDefault(id, Collections.emptyList());
            List<AssigneeVO> assigneeDTOList = activityDesignateAssigneeService.getAssigneesByActivityIds(List.of(id))
                    .getOrDefault(id, Collections.emptyList());
    ...
    public void removeByRepositoryId(Long repositoryId) {
        // 获取所有活动ID
        List<Long> activityIds = super.list(
                new QueryWrapper<ActivityPO>()
                        .select("id")
                        .eq("repository_id", repositoryId))
                .stream()
