...
@Service
public class ActivityDesignateAssigneeServiceImpl  extends ServiceImpl<ActivityDesignateAssigneeMapper, ActivityDesignateAssigneePO> implements ActivityDesignateAssigneeService{
    ...
    @Override
    public Page<AssigneeDTO> pageActivityAssigneesByActivityId(Long activityId, Page<AssigneeDTO> page) {
        var queryWrapper =
    ...
    @Override
    public ActivityDesignateAssigneePO getOneByActivityIdAndAssigneeId(Long activityId, Long assigneeId) {
        return super.getOne(
                new QueryWrapper<ActivityDesignateAssigneePO>()
                        .eq("activity_id", activityId)
    ...
    public Map<Long, List<AssigneeVO>> getAssigneesByActivityIds(List<Long> activityIds) {
            var queryWrapper = JoinWrappers.lambda(ActivityDesignateAssigneePO.class)
                    .selectAsClass(ActivityDesignateAssigneePO.class, AssigneeDTO.class)
                    .selectAs(UserPO::getUsername,AssigneeDTO::getUsername)
                    .selectAs(UserPO::getAvatarUrl,AssigneeDTO::getAvatarUrl)
                    .selectAs(UserPO::getEmail,AssigneeDTO::getEmail)
                    .innerJoin(
                            UserPO.class,
                            UserPO::getId,
                            ActivityDesignateAssigneePO::getAssigneeId)
                    .in(ActivityDesignateAssigneePO::getActivityId, activityIds);

            List<ActivityDesignateAssigneeDTO> assigneeDTOs = activityDesignateAssigneeMapper.selectJoinList(ActivityDesignateAssigneeDTO.class, queryWrapper);

            return assigneeDTOs.stream()
                    .collect(Collectors.groupingBy(
                            ActivityDesignateAssigneeDTO::getActivityId,
                            Collectors.mapping(
                                    dto -> new AssigneeVO(dto.getId().toString(),dto.getAssigneeId().toString(),dto.getUsername(), dto.getAvatarUrl(), dto.getEmail()),
                                    Collectors.toList()
                            )));
    }
