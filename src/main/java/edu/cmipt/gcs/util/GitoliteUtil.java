...
public class GitoliteUtil {
  ...
  public static synchronized boolean addSshKey(Long sshKeyId, String key, Long userId) {
    ...
    try {
      ...
      List<String> lines =
          Files.readAllLines(Paths.get(GitConstant.GITOLITE_USER_CONF_DIR_PATH, userFileName));
      for (int i = 0; i < lines.size(); i++) {
        String line = lines.get(i);
        if (line.startsWith("@%d_ssh_key".formatted(userId))) {
          lines.set(i, line + ' ' + sshKeyId);
          Files.write(Paths.get(GitConstant.GITOLITE_USER_CONF_DIR_PATH, userFileName), lines);
      ...
      for (int i = 0; i < lines.size(); i++) {
        ...
        if (line.startsWith("@%d_%s_repo".formatted(userId, isPrivate ? "private" : "public"))) {
          ...
          ProcessBuilder dirRemover =
              new ProcessBuilder(
                  "sudo", "-u", GitConstant.GIT_SERVER_USERNAME, "rm", "-rf", repositorySavePath);
  ...
  public static synchronized boolean addCollaborator(
    ...
    logger.error(
        "Can not find @{}_repo_collaborator in repository configuration".formatted(repositoryId));
    return false;
  }
    ...
    try {
      ...
      for (int i = 0; i < lines.size(); i++) {
        ...
        if (line.startsWith("@%d_repo_collaborator".formatted(repositoryId))) {
          ...
          Files.write(repositoryConfPath, lines);
          String message =
              "Remove collaborator " + collaboratorId + " from repository " + repositoryId;
          Path[] files = {Paths.get("conf", "gitolite.d", "repository", repositoryFileName)};
          if (!GitoliteUtil.commitAndPush(message, files)) {
            logger.error("Failed to commit and push");
            return false;
